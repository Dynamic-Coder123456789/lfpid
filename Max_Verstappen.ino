...
if (sum == 5) {
    ...
    error = 10000;      // small right bias instead of huge turn
    lastDirection = 1;
} 
else if (sum > 0) {
    ...
    // All white → lost line, keep last direction gently
    error = lastDirection * 500;  // smaller than before for smoother recovery
}
